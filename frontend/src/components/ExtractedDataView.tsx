import { Database, AlertTriangle, Lightbulb, BookOpen, Users, BarChart3, Zap, FileText, Calendar, TrendingUp } from 'lucide-react';
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <ResponsiveContainer width="100%" height={220}>
                            <PieChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                                <Pie
                                    ...
                                    dataKey="count"
                                    stroke="none"
                                    labelLine={{ stroke: 'var(--text-light)', strokeWidth: 1, strokeOpacity: 0.3 }}
                        ...
                        <div className="surface-neu p-8">
                            ...
                            <ResponsiveContainer width="100%" height={220}>
                                <BarChart data={confidenceData} barSize={24} margin={{ top: 20, right: 30, left: 0, bottom: 20 }}>
                                    <XAxis dataKey="name" tick={{ fill: 'var(--text-light)', fontSize: 11, fontFamily: 'Inter' }} axisLine={false} tickLine={false} dy={10} />
                                    <YAxis domain={[0, 100]} tick={{ fill: 'var(--text-light)', fontSize: 11 }} axisLine={false} tickLine={false} dx={-10} />
                                    <Tooltip
                                        formatter={(v) => [`${v}%`, 'Confidence']}
                                        contentStyle={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '0.75rem', color: 'var(--text-main)', fontFamily: 'Inter', padding: '12px' }}
                                    />
                    ...
                    <div className="surface-neu p-8">
                        ...
                        <div className="relative">
                            <ResponsiveContainer width="100%" height={220}>
                                <RadialBarChart cx="50%" cy="50%" innerRadius="70%" outerRadius="100%" data={radialData} startAngle={180} endAngle={0} margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
                                    <RadialBar background dataKey="value" cornerRadius={10} />
                                    <Legend iconSize={10} wrapperStyle={{ fontSize: 12, fontFamily: 'Inter', color: 'var(--text-light)', bottom: 0 }} />
                                    <Tooltip contentStyle={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '0.75rem', fontFamily: 'Inter', padding: '12px' }} />
                                </RadialBarChart>
                            </ResponsiveContainer>
                            <div className="absolute inset-0 flex flex-col pt-16 items-center justify-center pointer-events-none">
                                <p className="text-3xl font-extrabold text-gradient">{totalItems}</p>
                ...
                {methodologies.length === 0 ? (
                    ...
                    <div className="space-y-6">
                        {methodologies.map((m, i) => (
                            <div key={i} className="surface-neu-pressed p-8 rounded-2xl gradient-border">
                                <h4 className="font-bold text-sm text-primary mb-6 flex items-center gap-2">
                                    <Lightbulb className="w-4 h-4" /> Methodology {i + 1}
                                </h4>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-8">
                                    ...
                                    <div className="flex flex-col">
                                        <p className="text-[10px] font-bold text-textLight uppercase tracking-widest mb-3">Optimization</p>
                                        <div className="p-4 rounded-xl text-sm leading-relaxed" style={{ background: 'rgba(245,158,11,0.08)', color: '#f59e0b', border: '1px solid rgba(245,158,11,0.15)' }}>
                                            {m.optimization || 'N/A'}
                                        </div>
                                    </div>
                                </div>
...
function TagRow({ label, items, color }: { label: string; items: string[]; color: string }) {
    return (
        <div className="flex flex-col">
            <p className="text-[10px] font-bold text-textLight uppercase tracking-widest mb-3">{label}</p>
